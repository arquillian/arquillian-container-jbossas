 ...
 * <p>
 * Embedded uses MainDeployer which seems to not register the correct information with the ProfileService so we can
 * retrieve
 * it for inspection of Servlet etc. So we do not use embedded.deploy but rather lookup the ProfileService and deploy via
 * that.
 *
...
public class JBossASEmbeddedContainer implements DeployableContainer<JBossASContainerConfiguration> {
    ...
    public ProtocolMetaData deploy(final Archive<?> archive) throws DeploymentException {
        ...
        deploy(deploymentName, deploymentUrl);
        try {
            return ManagementViewParser.parse(archive.getName(),
                (ProfileService) new InitialContext().lookup("ProfileService"));
        } catch (Exception e) {
